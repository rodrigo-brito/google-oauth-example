...
var (
	...
	oAuthPort         = flag.Int("oAuthPort", 8090, "TCP port to listen on when requesting an oAuth token")
...
func readConfig(scopes []string) (*oauth2.Config, error) {
	...
	if len(cfg2.RedirectURIs) > 0 {
		...
		fmt.Printf("Redirect URL could not be found. Using default: http://localhost:8090/oauth2callback")
		redirURL = "http://localhost:8090/oauth2callback"
	}
...
func authFlow(ctx context.Context, scopes ...string) error {
	...
	if err != nil {
		...
		if cbs.state != randState {
			return fmt.Errorf("expecting state '%s', received state '%s'", randState, cbs.state)
		}
		...
		fmt.Println("Token already downloaded: ", *cache)
	}
...
func main() {
	...
	err := authFlow(ctx, calendar.CalendarScope)
	if err != nil {
